...
import 'package:frolicsports/models/playersAllPointsResponce.dart';
...
import 'package:frolicsports/modules/home/FantasyPointSystemScreen.dart';
import 'package:frolicsports/utils/avatarImage.dart';
import 'package:frolicsports/validator/validator.dart';
import 'package:frolicsports/models/leagueMemberTeamResponse.dart'
    as leagueMember;
...
class _LeagueTeamPreviewScreenState extends State<LeagueTeamPreviewScreen> {
  ...
  final teamSelectionBloc = TeamSelectionBloc();
  final teamTapBloc = TeamTapBloc();
  var isProsses = false;
  var leagueTeamData = leagueMember.Leagues();
  var isEdite = false;
  var playingList = List<Playing11>();
      ...
      if (data != null && data.success == 1 && data.playerList.length > 0) {
        ...
        allPlayerList.forEach((p) {
          if (p.title.toUpperCase() ==
              leagueTeamData.leagueMember[0].captun.toUpperCase()) {
            p.isSelcted = true;
            p.isC = true;
          }
          if (p.title.toUpperCase() ==
              leagueTeamData.leagueMember[0].wiseCaptun.toUpperCase()) {
            p.isVC = true;
            p.isSelcted = true;
          }
          if (isMach(p.title)) {
        ...
        leagueTeamData.playerScore.forEach((score) {
          allPlayerList.forEach((p) {
            if (p.shortName.toUpperCase() == score.playerName.toUpperCase() &&
                    p.pid == int.tryParse(score.playerKey) ??
                0) {
              p.point = score.point;
            }
          });
        ...
        teamSelectionBloc.onListChanges(allPlayerList, context.bloc<SportsDataBloc>().getTournamentSkills(widget.matchDataForApp.sportsName, widget.matchDataForApp.tournamentName));
      }
      setState(() {
        isProsses = false;
      });
                ...
                child: Stack(
                  ...
                  children: <Widget>[
                    ...
                    Column(
                      children: <Widget>[
                        Expanded(
                          child: BlocBuilder<TeamSelectionBloc, TeamSelectionBlocState>(
...
//                                  .compareTo(a.fantasyPlayerRating));
  ...
  bool isMachStart() {
    var finaledate = DateFormat('yyyy-MM-dd HH:mm:ss').parse(
        widget.matchDataForApp.startDate + " " + widget.matchDataForApp.startTime);
//        widget.shedualData.dateStart + " " + widget.shedualData.timeStart);
  ...
  Widget getTypeList(List<Players> list) {
    ...
    //           mainAxisSize: MainAxisSize.min,
    //           children: <Widget>[
    //             pList.length >= 1 ? pList[0] : Container(),
    //             pList.length >= 2 ? pList[1] : Container(),
    //             // pList[2],
    //           ],
    //         ),
    //         Row(
    ...
    //           mainAxisSize: MainAxisSize.min,
    //           children: <Widget>[
    //             pList.length >= 3 ? pList[2] : Container(),
    //             pList.length >= 4 ? pList[3] : Container(),
    //           ],
    //         )
    //       ],
    //     );
